/* ----------------------------------------------------------------------------
 * This file was automatically generated by SWIG (http://www.swig.org).
 * Version 3.0.8
 *
 * Do not make changes to this file unless you know what you are doing--modify
 * the SWIG interface file instead.
 * ----------------------------------------------------------------------------- */

package nz.mega.sdk;

public class MegaNodeList {
  private transient long swigCPtr;
  protected transient boolean swigCMemOwn;

  protected MegaNodeList(long cPtr, boolean cMemoryOwn) {
    swigCMemOwn = cMemoryOwn;
    swigCPtr = cPtr;
  }

  protected static long getCPtr(MegaNodeList obj) {
    return (obj == null) ? 0 : obj.swigCPtr;
  }

  protected void finalize() {
    delete();
  }

  protected synchronized void delete() {   
    if (swigCPtr != 0) {
      if (swigCMemOwn) {
        swigCMemOwn = false;
        megaJNI.delete_MegaNodeList(swigCPtr);
      }
      swigCPtr = 0;
    }
}

  public static MegaNodeList createInstance() {
    long cPtr = megaJNI.MegaNodeList_createInstance();
    return (cPtr == 0) ? null : new MegaNodeList(cPtr, false);
  }

   MegaNodeList copy() {
    long cPtr = megaJNI.MegaNodeList_copy(swigCPtr, this);
    return (cPtr == 0) ? null : new MegaNodeList(cPtr, true);
  }

  public MegaNode get(int i) {
    long cPtr = megaJNI.MegaNodeList_get(swigCPtr, this, i);
    return (cPtr == 0) ? null : new MegaNode(cPtr, false);
  }

  public int size() {
    return megaJNI.MegaNodeList_size(swigCPtr, this);
  }

  public void addNode(MegaNode node) {
    megaJNI.MegaNodeList_addNode(swigCPtr, this, MegaNode.getCPtr(node), node);
  }

  public MegaNodeList() {
    this(megaJNI.new_MegaNodeList(), true);
  }

}
